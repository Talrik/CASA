...
class CourseNavigation extends Navigation
{
    ...
    public function initSubNavigation()
    {
		...
		/*	
		 *	CHANGED!!!
		 *	Hier: Reiter fÃ¼r Dienste
		 *	Bedingung: Broker erreichbar
		 *	Bedingung: Tutor, Dozent
		 *	Dazu: Uebergabe der resource_id und Nutzerrolle
		 */
		
		
        
//        $fp = @fsockopen("elbe5:8080/Broker4GUI", 80);	// Prï¿½fen ob der Broker erreichbar ist
	  $fp = @fsockopen("localhost:8080/GUI_Broker", 80);
        if ($fp) {
            //  RaumID ermitteln
            $sem = Seminar::GetInstance($SessSemName[1]);
            $uniqueSemId = $sem->id;							// SeminarID
            $nextDateDB = SeminarDB::getNextDate($uniqueSemId);	// nï¿½chster Termin vom Seminar
            $uniqueDateId = $nextDateDB['termin'][0];			// ID vom nï¿½chsten Termin
            $db = DBManager::get();
            ...
            $uniqueResourceId = $fetchedSearched[0];			// RaumID vom nï¿½chsten Termin
	    ...
	    $parentId = $fetchedSearched[0];
	    $nameSearch = $db->query("SELECT `name` FROM `resources_objects` WHERE `resource_id` = '08e2db1aff87f8e3a16b11074fb57b90' OR `resource_id` = '6a20c8ed5a53d7331d759e14efaa2cfa'");
	    $fetchedSearched = $nameSearch->fetch();
	    $roomName =  $fetchedSearched[0];
	    $fetchedSearched = $nameSearch->fetch();
	    $locationName =  $fetchedSearched[0];
...
//KÃ¼rzen	
	...
	$fp = @fsockopen("http://localhost:8080/GUI_Broker",8080);
//	echo var_dump($fp);
try{
//	if ($fp){
