...
class CourseNavigation extends Navigation
{
    ...
    public function initSubNavigation()
    {
		...
		/*
		 *	CHANGED!!!
		 *	Hier: Reiter fuer Dienste
		 *	Bedingung: Broker erreichbar
		 *	Bedingung: Tutor, Dozent, Admin
		 *	Dazu: Uebergabe der resource_id und Nutzerrolle
		 */

	// Prüfen ob der Broker erreichbar ist
	  $fp = @fsockopen("elbe5:8080/GUI_Broker", 80);
       if ($fp) {
            //  RaumID ermitteln
            	$sem = Seminar::GetInstance($SessSemName[1]);
            	$uniqueSemId = $sem->id;				// SeminarID
            	$nextDateDB = SeminarDB::getNextDate($uniqueSemId);	// naechster Termin vom Seminar
...
//				"userID" => $username,
			...
			//  Wenn Dienste für die Anfrage vorliegen wird die Navigation um den Reiter Dienste erweitert
        		if(sizeof($array) == 1){
           			$title = $array->title;
				$ac = 0;
				while ($ac < count($array->properties->entry)){
					if ($array->properties->entry[$ac]->key === "targetURL"){
						$url = $array->properties->entry[$ac]->value;
					} 
					$ac++;
				}
				...
				//  Wenn der Nutzer eine leitende Funktion in der Veranstaltung hat darf er auch Dienste eintragen
				//  Hunzufuegen der SubNavigation zum Eintragen von Diensten
 				if(($role == 'admin') || $role == 'dozent' || $role == 'tutor'){
			...
			if (sizeof($array) >1){
				...
				for($i = 0; $i< sizeof($array);$i++){
 					$ac = 0;
                                	while ($ac < count($array[$i]->properties->entry)){
                                        	if ($array[$i]->properties->entry[$ac]->key === "targetURL"){
                                                	$url = $array[$i]->properties->entry[$ac]->value;
                                        	}
                                        	$ac++;
                                	}
					$services = $services.'&servicetitle[]='.$array[$i]->title.'&serviceurl[]='.urlencode($url);
				}
		...
		// Falls es Probleme mit dem WebService und seiner Antwort gibt kann der Fehler ausgegeben werden
		catch (SoapFault $E){
		//	echo $E;
		}
	}	//CASA Ende
