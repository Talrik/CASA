...
/**
 * Fact entry for {@link ContextServer} implemented as Camel Processor 
 *
 */
public class FactEntry implements Processor{
	...
	public void process(Exchange exchange) throws Exception {
		log.debug("Msg received by FE "+ exchange + " from "+ exchange.getFromEndpoint());
		log.debug("FE: "+exchange.getIn().getBody() );
		Object e = exchange.getIn().getBody();
		if (server == null) {System.err.println("[FE]: No ContextServer available");}		
		else if (e instanceof Entity){
			Entity ent = (Entity)e;
			server.addEntity(ent);
			log.debug("FE: "+ent.getClass() +" : "+ent.getProperties() );
