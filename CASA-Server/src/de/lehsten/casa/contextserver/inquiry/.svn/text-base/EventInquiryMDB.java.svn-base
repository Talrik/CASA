...
import de.lehsten.casa.contextserver.DroolsContextServer;
...
//    StatefulKnowledgeSession ksession;
    ...
    public EventInquiryMDB() { 
    	try {
			...
			cs = (ContextServer) ctx.lookup("java:global/CASA-Server/CASAContextServer");
//			ksession = cs.getSession(); 
	...
	public void onMessage(Message message) {
		try {
			 ...
			 */
/*			if (cs == null) {System.err.println("[EI]: No knowledge session available");}
...
/*
				...
				if (cs.getQueryNames().contains(msg.text)){
					
					EntityResp = (ArrayList<Entity>) cs.getQueryResult(msg.text, msg.parameter.values().toArray());
					CSMessage responseMessage = new CSMessage();
					if (EntityResp != null) {
						responseMessage.text = EntityResp.size()
								+ " entities(s) found!";
					...
					System.out.println("[EI]:Request ["+msg.text+"] does not match known requests.");
					System.out.println("[EI]:Known request: "+cs.getQueryNames());
					...
					System.out.println("[EI]: Request unrecognized! Message corrupted?");
				}
...
//			System.out.println(lecture.getTitle() +", "+ new Date(lecture.getBegin()*1000));
	...
	private static ArrayList<Course> GetNextCourseByRoom(String room, Long date ,StatefulKnowledgeSession ksession){
		...
		for ( QueryResultsRow row : results ) {
			course = ( Course) row.get( "c" );
			lecture = (Lecture) row.get("l");
			System.out.println(course.getTitle() +", hat in "+ room+" eine Veranstaltung am "+ lecture.getBeginDate());
			list.add(course);
		}
		return list;
	...
	private static ArrayList<Course> GetNextCourseByUsername(String username, Long date ,StatefulKnowledgeSession ksession){
		...
		for ( QueryResultsRow row : results ) {
			...
			course = ( Course) row.get( "c" );
			lecture = (Lecture) row.get("l");
			System.out.println(person.getFirstname() +", hat "+ lecture.getBeginDate() +" die Veranstaltung "+ lecture.getTitle() + " in " + lecture.getRoom().getTitle() );
			list.add(course);
		}
		return list;
	...
	private static ArrayList<Course> GetActualCourseByRoom (String room, Long date ,StatefulKnowledgeSession ksession){
		...
		for ( QueryResultsRow row : results ) {
			...
			course = ( Course) row.get( "c" );
			lecture = (Lecture) row.get("l");
			System.out.println("In "+temp_room.getTitle() +" takes  "+ lecture.getTitle() + " since "+ lecture.getBeginDate() +" place" );
			list.add(course);
		}
		return list;
