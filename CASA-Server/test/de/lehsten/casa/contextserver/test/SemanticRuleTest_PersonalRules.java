...
public class SemanticRuleTest_PersonalRules {
...
*/	
	@Test
	public void RequestTest_MensaService() {
		// config the query
		...
		Position currentPos = new Position(currentLatitude,currentLongitude);
		qrequest.getRestrictions().add(currentPos);
		//insert objects
		ksession.insert(w1);
		ksession.insert(qrequest);
		//fire query
		int rulesFiredSecond = ksession.fireAllRules( new RuleNameEqualsAgendaFilter( "MensaService" ) );
		assertEquals(1, rulesFiredSecond);
		//get the request back out of the session
		queryName = "GetRequestById";
		arguments = new Object[1];
